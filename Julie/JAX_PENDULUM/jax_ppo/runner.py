...
def train(
    ...
    **static_kwargs,
    ...
    def _train_step(carry, _):
        ...
        state_seqs.append(_obs_ts)
        return (_key, _agent), (_losses, _obs_ts, _rewards_ts, _info_ts, state_seqs)

    (key, agent), (losses, obs_ts, rewards_ts, info_ts, state_seqs) = jax.lax.scan(
        _train_step, (key, agent), jnp.arange(n_train)
    )

    return key, agent, losses, obs_ts, rewards_ts, info_ts, state_seqs