...
if __name__ == "__main__":
    ...
    params = jax_ppo.default_params._replace(
        gamma=0.99, gae_lambda=0.99, entropy_coeff=0.001, adam_eps=1e-8, clip_coeff=0.25
    )
    ...
    plt.show()
    new_arr = jnp.mean(jnp.sum(cumulative_rewards[:, :, :], axis=2), axis=1)
    print(new_arr.shape)

    # Visualize the recorded states and rewards
    # vis = Visualizer(env, env_params, state_seq, cumulative_rewards)
    doc_path = "docs/anim.gif"
    visualization.visualize(env, env_params, k, doc_path, new_arr, state_seq)
